...
def limits(ex, symbol):
    ...
    amt = lim.get("amount", {})
    cost = lim.get("cost", {})
    return float(amt.get("min") or 0), float(cost.get("min") or 0)
...
def render_header(current_balance):
    ...
    print(f"🏦 Saldo atual: {current_balance:.4f} USDT")
    print("========================================")
...
def execute_cycle(ex, plan):
    ...
    base = plan["base"]
    quote2 = plan["quote2"]
    print(f"🟢 Executando: USDT → {base} → {quote2} → USDT | Spread líquido: {plan['spread_net']:.2f}%")
    ...
    try:
        ...
        msg = (
            ...
            f"💵 Lucro neste ciclo: {gain:.4f} USDT\n"
