...
def render_header(current_balance):
    ...
    print("========================================")
    print("📊 GENERAL STATUS — MEXC Arbitrage (Spot)")
    print("----------------------------------------")
    print(f"🕒 Uptime: {dur}")
    print(f"🔁 Completed cycles: {cycles}")
    print(f"💰 Accumulated profit: {total_profit_usdt:.4f} USDT ({total_pct:.2f}%)")
    print(f"🏦 Estimated USDT balance: {current_balance:.4f} USDT")
    print("========================================")
...
def plan_from_cache(prices, ex, usdt_avail, base, quote2):
    ...
    s2 = f"{base}/{quote2}"
    s3 = f"{quote2}/USDT"
    t1, t2, t3 = prices.get(s1), prices.get(s2), prices.get(s3)
    ...
    return {"spread_net": net_spread_pct, "s1": s1, "s2": s2, "s3": s3,
            "a1": a1, "b2": b2, "b3": b3, "usdt_use": usdt_use,
            "qty_base": qb, "qty_quote2": qq2, "base": base, "quote2": quote2}
...
def execute_cycle(ex, plan):
    ...
    base = plan["base"]
    quote2 = plan["quote2"]
    print(f"🟢 Executing: USDT → {base} → {quote2} → USDT | Net spread: {plan['spread_net']:.2f}%")
